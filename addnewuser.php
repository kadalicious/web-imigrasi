    ...
    if(!empty(trim($password))){
      if(!empty(trim($ulangpassword))){
        if($password == $ulangpassword){
          ...
          $insertdata = "INSERT INTO users set username = '$username', password = '$passwordencrypt', role = '$role'";
          $connectinsertdata = mysqli_query($koneksi, $insertdata);
          if($connectinsertdata == true){
            ?>
            <script type="text/javascript">
              alert("berhasil menambahkan user baru, tunggu 2 detik untuk berpindah ke halaman login");
...
<html lang="en">
  ...
  <body>
    ...
    <div style="display: flex; justify-content: center;margin-top: 50px;">
      <h1 style="font-weight: bolder;">TAMBAH USER BARU</h1>
    </div>
    ...
    <form method="POST" action="">
      ...
      <!-- Submit button -->
      <input type="submit" name="submit" value="Tambah User" class="btn btn-primary" style="width: 100%; "/>
